...
// Lidar odometry mod varun 
/*
template<typename PointT>
typename pcl::PointCloud<PointT>::Ptr 
 Filters<PointT>::VoxelGridDownSampling(const typename pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud2, 
                                        const float &filterRes){
    // DownSample the dataset using a given leaf size
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud2_filtered(new pcl::PointCloud<pcl::PointXYZ>); // Create filtered object
    ...
    voxelFilter.filter(*cloud2_filtered);
    //pcl::fromPCLPointCloud2(*cloud2_filtered, *cloud_filtered); // PCLPointCloud2 ---> pcl::PointXYZ
    std::cout << "[VoxelGridDownSampling]  Original points: " << cloud2->width * cloud2->height <<  ", Filtered points: " << cloud2_filtered->points.size() << std::endl;
    return cloud2_filtered;
} // this function structure looks like a shit.... ,(O∆O), */
